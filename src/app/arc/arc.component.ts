import { BoundElementProperty, devOnlyGuardedExpression } from '@angular/compiler';
...
export class ArcComponent implements OnInit {
  ...
  private drawArc(nodes :any , links: any): void {
    const color = d3.scaleOrdinal(nodes.map(d => d.group).sort(d3.ascending), d3.schemeCategory10);
    const y = d3.scalePoint(nodes.map(d => d.id).sort(d3.ascending), [this.margin.top, this.height - this.margin.bottom]);
    const linkWidth = d3.scalePoint(links.map(d => d.value).sort(d3.ascending), [1.5, 7.0]);
    ...
    const label = this.svg.append("g")
      ...
      .data(nodes)
      .join("g")
        .attr("transform", d => `translate(${this.margin.left * 2},${d.y = y(d.id)})`)
    ...
    const path = this.svg.insert("g", "*")
      ...
      .data(links)
      .join("path")
        .attr("stroke", d => d.source.group === d.target.group ? color(d.source.group) : noMatchColour)
        .attr('stroke-width', d => linkWidth(d.source.group))
        .attr("stroke-opacity", 0.6)
        .attr("d", this.arc)
        .attr('class', d => d.source.id.replace(/\./g,' ') + " " + d.target.id.replace(/\./g,' ') + " p" + d.source.group);
    ...
    const overlay = this.svg.append("g")
      ...
      .join("rect")
        ...
        .on("mouseover", (event, d) => {
          ...
          for(let source of sources){
            this.svg.selectAll(`circle.${source[0]}`)
              .attr('stroke-width', '1')
              .attr("stroke",'black')
            this.svg.selectAll(`text.${source[0]}`)
              .attr('font-weight', "bold")
              .attr("font-size", 15)
          }
          for(let target of targets){
            this.svg.selectAll(`circle.${target[0]}`)
              .attr('stroke-width', '1')
              .attr("stroke",'black')
            this.svg.selectAll(`text.${target[0]}`)
              .attr('font-weight', "bold")
              .attr("font-size", 15)
          }
          this.svg.selectAll(`text.${d.id}`)
            .attr('font-weight', "bold")
            .attr("font-size", 15)
        })
        .on("mouseout", (event, d) => {
      ...
      const legend = this.svg.append("g")
          ...
          .data(grouparr)
          .join("g")
            .attr("transform", d => `translate(${this.width - this.width / 2},${(d * 10) + 5})`)
            .call(g => g.append("text")
              .attr("x", 6)
              .attr("dy", "0.35em")
              .attr('class', d => "p" + d)
            ...
            .call(g => g.append("circle")
              ...
              .on("click", (event, d) => {
                ...
                if(stateArray[d] === true){
                  stateArray[d] = false
                  this.svg.selectAll(`path.p${d}`)
                    .attr("stroke-opacity", 0.6)
                    .attr("stroke", d => d.source.group === d.target.group ? color(d.source.group) : "#aaa");
                  this.svg.selectAll(`circle.p${d}`)
                    .attr('stroke-width', '0')
                    .attr("stroke", color(d))
                  this.svg.selectAll(`text.p${d}`)
                    .attr("fill", d3.lab(color(d)))
                    .attr('font-weight', "none")
                    .attr("font-size", 10)
                }
                else if(stateArray[d] === false){
                  d3.selectAll(`path:not(.p${d})`)
